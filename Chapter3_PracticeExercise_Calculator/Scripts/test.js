...
test("Button Click Test", function () {
    ...
    for (var i = 0; i < buttonQuantity; i++) {
        ...
        var result = txtInput.value[txtInput.value.length - 1]; // gets the last number clicked (last number in string of input)
...
test('Add Test', function () {
    ...
    var expected = '30';
    equal(txtResult.value, expected, 'Exepected value: ' + expected +
        ' Actual value: ' + txtResult.value);
    expected = '0';
    equal(txtInput.value, expected, 'Expected value: ' + expected
        + ' Actual value: ' + txtInput.value);
